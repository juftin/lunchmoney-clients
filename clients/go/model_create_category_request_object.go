...
type CreateCategoryRequestObject struct {
	// Name of the new category. Must be between 1 and 100 characters. The name must not match the name of any existing categories or category groups.
	Name string `json:"name"`
	// Description of the category. Maximum length is 200 characters.
	...
	// If `true`, this category will be created as a category group.
	IsGroup *bool `json:"is_group,omitempty"`
	// If set to the ID of an existing category group, the new category will be added to that group. Cannot be used if `is_group` is true.
	GroupId NullableInt64 `json:"group_id,omitempty"`
	// If `true`, the category is archived and in relevant areas of the Lunch Money app.
	Archived *bool `json:"archived,omitempty"`
	// List of categories to include in the new category group. This field should only be set if `is_group` is also set to true.<br> You may provide existing category objects, existing category IDs, or names for new categories to add to the group. Categories or IDs must already exist and cannot be category groups. Categories that already belong to another group will be moved. If strings are provided, they will be used as names for new categories added to the group. The request will fail if any provided name already exists.<br> You may mix category objects, IDs, and new category names in the same request.
	Children []CreateCategoryRequestObjectChildrenInner `json:"children,omitempty"`
	// Position of the category on the categories page in the Lunch Money app. For grouped categories, the order is relative to other categories in the same group.<br>While this property can be set via the API, it is usually managed by the user in the Lunch Money app.
	Order NullableInt32 `json:"order,omitempty"`
	// If `true`, the category group appears collapsed in the Lunch Money app. Can only be set to `true` for category groups.<br>While this property can be set via the API, it is usually managed by the user in the Lunch Money app.
...
func (o *CreateCategoryRequestObject) SetOrderNil() {
	o.Order.Set(nil)
}
...
func (o *CreateCategoryRequestObject) SetCollapsedNil() {
	o.Collapsed.Set(nil)
}

...
class BalanceHistoryListResponseObject(BaseModel):
    """
    List response for balance history GET endpoints. Entries are grouped by account source under `balance_history`. 
    """ # noqa: E501
    balance_history: List[BalanceHistoryAccountObject]
    __properties: ClassVar[List[str]] = ["balance_history"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in balance_history (list)
        _items = []
        if self.balance_history:
            for _item_balance_history in self.balance_history:
                if _item_balance_history:
                    _items.append(_item_balance_history.to_dict())
            _dict['balance_history'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "balance_history": [BalanceHistoryAccountObject.from_dict(_item) for _item in obj["balance_history"]] if obj.get("balance_history") is not None else None
        })
        return _obj

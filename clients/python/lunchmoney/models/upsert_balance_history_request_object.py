...
class UpsertBalanceHistoryRequestObject(BaseModel):
    """
    UpsertBalanceHistoryRequestObject
    """ # noqa: E501
    balances: Annotated[List[BalanceHistoryUpdateItemObject], Field(min_length=1)] = Field(description="One or more monthly balance history entries to upsert. Each entry uses `month` (YYYY-MM) and `balance`. Do not include response-only fields such as `type`. PUT responses return only the `type: historical` entries modified by the request. ")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "balances": [BalanceHistoryUpdateItemObject.from_dict(_item) for _item in obj["balances"]] if obj.get("balances") is not None else None
        })
        return _obj

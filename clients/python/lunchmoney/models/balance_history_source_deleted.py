...
class BalanceHistorySourceDeleted(BaseModel):
    """
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Identifies this entry as belonging to an account that has since been deleted.")
    deleted_account_id: StrictInt = Field(description="Identifier for the deleted account history source")
    name: Optional[StrictStr] = Field(description="Archived account `name` for the deleted account source")
    institution_name: Optional[StrictStr] = Field(description="Archived `institution_name` for the deleted account source")
    display_name: Optional[StrictStr] = Field(description="Archived `display_name` of the deleted account")
    account_type: Optional[StrictStr] = Field(description="Archived `type` of the deleted account source")
    subtype: Optional[StrictStr] = Field(description="Archived `subtype` of the deleted account source")
    mask: Optional[StrictStr] = Field(description="Archived account `mask` for a deleted Plaid account source")
    symbol: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=25)]] = Field(description="Archived `symbol` for a deleted crypto account source")
    __properties: ClassVar[List[str]] = ["type", "deleted_account_id", "name", "institution_name", "display_name", "account_type", "subtype", "mask", "symbol"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "deleted_account_id": obj.get("deleted_account_id"),
            "name": obj.get("name"),

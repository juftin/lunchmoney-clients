...
class HistoricalBalanceHistoryEntry(BaseModel):
    """
    ...
    """ # noqa: E501
    type: StrictStr = Field(description="Identifies this entry as a stored snapshot of a past month.")
    id: StrictInt = Field(description="Unique identifier for this historical balance entry.")
    month: Annotated[str, Field(strict=True)] = Field(description="Calendar month for this entry in YYYY-MM format.")
    balance: Annotated[str, Field(strict=True)] = Field(description="Historical balance for this entry, as a numeric string with up to four decimal places. Trailing zeros and decimal places are not guaranteed in responses. For manual and Plaid accounts this is in the account currency. For crypto accounts this is in the user's primary currency.")
    currency: CurrencyEnum = Field(description="Currency of `balance`. For crypto entries this is the user's primary currency.")
    to_base: Union[StrictFloat, StrictInt] = Field(description="Historical balance converted to the user's primary currency. When the entry currency is the user's primary currency, this is the numeric value of `balance`.")
    crypto_balance: Optional[Annotated[str, Field(strict=True)]] = Field(description="Crypto quantity for this balance entry, when available. This may be present for crypto or deleted-account entries and is `null` otherwise.")
    __properties: ClassVar[List[str]] = ["type", "id", "month", "balance", "currency", "to_base", "crypto_balance"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['historical']):
            raise ValueError("must be one of enum values ('historical')")
        return value

...
class BalanceHistorySourcePlaid(BaseModel):
    """
    Source information for a Plaid-synced account balance history entry.
    """ # noqa: E501
    type: StrictStr = Field(description="Identifies this entry as belonging to a Plaid-synced account.")
    plaid_account_id: StrictInt = Field(description="ID of the Plaid account associated with this entry.")
    __properties: ClassVar[List[str]] = ["type", "plaid_account_id"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['plaid']):

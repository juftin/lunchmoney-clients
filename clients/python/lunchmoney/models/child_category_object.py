...
class ChildCategoryObject(BaseModel):
    ...
    order: Optional[StrictInt] = Field(description="An  specifying the position in which the category is displayed on the categories page in the Lunch Money GUI. For categories within a category group the order  is relative to the other categories within the group.<br> This property cannot be set or updated via the API.")
    __properties: ClassVar[List[str]] = ["id", "name", "description", "is_income", "exclude_from_budget", "exclude_from_totals", "updated_at", "created_at", "group_id", "is_group", "archived", "archived_at", "order"]

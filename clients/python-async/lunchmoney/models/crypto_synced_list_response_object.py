...
class CryptoSyncedListResponseObject(BaseModel):
    """
    CryptoSyncedListResponseObject
    """ # noqa: E501
    crypto_synced: List[SyncedCryptoAccount]
    __properties: ClassVar[List[str]] = ["crypto_synced"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in crypto_synced (list)
        _items = []
        if self.crypto_synced:
            for _item_crypto_synced in self.crypto_synced:
                if _item_crypto_synced:
                    _items.append(_item_crypto_synced.to_dict())
            _dict['crypto_synced'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "crypto_synced": [SyncedCryptoAccount.from_dict(_item) for _item in obj["crypto_synced"]] if obj.get("crypto_synced") is not None else None
        })
        return _obj

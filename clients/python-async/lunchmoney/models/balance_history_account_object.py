...
class BalanceHistoryAccountObject(BaseModel):
    """
    Monthly balance entries grouped under a single account source.
    """ # noqa: E501
    source: BalanceHistoryAccountObjectSource
    balances: List[BalanceHistoryEntry] = Field(description="Monthly balance entries for this account source. A `historical` entry is a stored snapshot of a past month and includes an `id`. A `current` entry is an ephemeral snapshot based on the account's current balances and has no balance-entry `id`. On PUT upsert responses, this array includes only the `type: historical` entries modified by that request. ")
    __properties: ClassVar[List[str]] = ["source", "balances"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "source": BalanceHistoryAccountObjectSource.from_dict(obj["source"]) if obj.get("source") is not None else None,
            "balances": [BalanceHistoryEntry.from_dict(_item) for _item in obj["balances"]] if obj.get("balances") is not None else None
        })
        return _obj

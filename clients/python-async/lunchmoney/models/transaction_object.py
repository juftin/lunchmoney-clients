...
class TransactionObject(BaseModel):
    ...
    TransactionObject
    """ # noqa: E501
    id: StrictInt = Field(description="System-created unique identifier for the transaction")
    var_date: date = Field(description="Transaction date in ISO 8601 format", alias="date")
    ...
    payee: StrictStr = Field(description="Name of payee set by the user, the financial institution, or by a matched recurring item. This will match the value displayed in payee field on the transactions page in the Lunch Money app. ")
    ...
    plaid_account_id: Optional[StrictInt] = Field(description="The unique identifier of the plaid account associated with this transaction. This will always be null if this transaction is associated with a manual account or if this transaction has no associated account and appears as a \"Cash Transaction\" in the Lunch Money app.")
    manual_account_id: Optional[StrictInt] = Field(description="The unique identifier of the manual account associated with this transaction. This will always be null if this transaction is associated with a synced account or if this transaction has no associated account and appears as a \"Cash Transaction\" in the Lunch Money app.")
    external_id: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=75)]] = Field(description="A user-defined external ID associated with the transaction. For transactions belonging to manual accounts, the external ID must be unique for each transaction associated with the account.")
    ...
    notes: Optional[StrictStr] = Field(description="Any transaction notes set by the user or by a matched recurring item. This will match the value displayed in notes field on the transactions page in the Lunch Money app. ")
    ...
    files: Optional[List[TransactionAttachmentObject]] = Field(default=None, description="A list of objects that describe any attachments to the transaction. This is only present when the `include_files` query parameter is set to true.")
    ...
    __properties: ClassVar[List[str]] = ["id", "date", "amount", "currency", "to_base", "recurring_id", "payee", "original_name", "category_id", "plaid_account_id", "manual_account_id", "external_id", "tag_ids", "notes", "status", "is_pending", "created_at", "updated_at", "is_split_parent", "split_parent_id", "is_group_parent", "group_parent_id", "children", "plaid_metadata", "custom_metadata", "files", "source"]

...
BALANCEHISTORYENTRY_ONE_OF_SCHEMAS = ["CurrentBalanceHistoryEntry", "HistoricalBalanceHistoryEntry"]
...
class BalanceHistoryEntry(BaseModel):
    ...
    """
    # data type: HistoricalBalanceHistoryEntry
    oneof_schema_1_validator: Optional[HistoricalBalanceHistoryEntry] = None
    # data type: CurrentBalanceHistoryEntry
    oneof_schema_2_validator: Optional[CurrentBalanceHistoryEntry] = None
    actual_instance: Optional[Union[CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry]] = None
    one_of_schemas: Set[str] = { "CurrentBalanceHistoryEntry", "HistoricalBalanceHistoryEntry" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BalanceHistoryEntry.model_construct()
        error_messages = []
        match = 0
        # validate data type: HistoricalBalanceHistoryEntry
        if not isinstance(v, HistoricalBalanceHistoryEntry):
            error_messages.append(f"Error! Input type `{type(v)}` is not `HistoricalBalanceHistoryEntry`")
        else:
            match += 1
        # validate data type: CurrentBalanceHistoryEntry
        if not isinstance(v, CurrentBalanceHistoryEntry):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CurrentBalanceHistoryEntry`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BalanceHistoryEntry with oneOf schemas: CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BalanceHistoryEntry with oneOf schemas: CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into HistoricalBalanceHistoryEntry
        try:
            instance.actual_instance = HistoricalBalanceHistoryEntry.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into CurrentBalanceHistoryEntry
        try:
            instance.actual_instance = CurrentBalanceHistoryEntry.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BalanceHistoryEntry with oneOf schemas: CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BalanceHistoryEntry with oneOf schemas: CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], CurrentBalanceHistoryEntry, HistoricalBalanceHistoryEntry]]:

...
class BalanceHistorySourceCryptoSynced(BaseModel):
    """
    Source information for a synced cryptocurrency balance history entry.
    """ # noqa: E501
    type: StrictStr = Field(description="Identifies this entry as belonging to a synced crypto connection.")
    crypto_synced_id: StrictInt = Field(description="ID of the synced crypto connection associated with this entry.")
    symbol: Annotated[str, Field(min_length=1, strict=True, max_length=25)] = Field(description="Crypto symbol (e.g. `eth`, `btc`) identifying the specific currency within the synced account.")
    __properties: ClassVar[List[str]] = ["type", "crypto_synced_id", "symbol"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['crypto_synced']):
            raise ValueError("must be one of enum values ('crypto_synced')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "crypto_synced_id": obj.get("crypto_synced_id"),

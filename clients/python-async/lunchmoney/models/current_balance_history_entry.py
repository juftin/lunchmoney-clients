...
class CurrentBalanceHistoryEntry(BaseModel):
    """
    An ephemeral snapshot based on the account's current balances. It may change between requests. 
    """ # noqa: E501
    type: StrictStr = Field(description="Identifies this entry as an ephemeral current-month snapshot.")
    month: Annotated[str, Field(strict=True)] = Field(description="Calendar month for this entry in YYYY-MM format. For current entries this is the current month.")
    balance: Annotated[str, Field(strict=True)] = Field(description="Calculated balance for the current month, as a numeric string with up to four decimal places. Trailing zeros and decimal places are not guaranteed in responses. For manual and Plaid accounts this is in the account currency. For crypto accounts this is in the user's primary currency.")
    currency: CurrencyEnum = Field(description="Currency of the calculated `balance`. For crypto entries this is the user's primary currency.")
    to_base: Union[StrictFloat, StrictInt] = Field(description="Calculated balance converted to the user's primary currency. When the entry currency is the user's primary currency, this is the numeric value of `balance`.")
    crypto_balance: Optional[Annotated[str, Field(strict=True)]] = Field(description="Crypto quantity for this calculated entry, when available. This may be present for crypto entries and is `null` otherwise.")
    __properties: ClassVar[List[str]] = ["type", "month", "balance", "currency", "to_base", "crypto_balance"]

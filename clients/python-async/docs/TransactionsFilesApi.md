...
async with lunchmoney.ApiClient(configuration) as api_client:
    ...
    try:
        # Attach a file to a transaction
        api_response = await api_instance.attach_file_to_transaction(transaction_id, file, notes=notes)
...
async with lunchmoney.ApiClient(configuration) as api_client:
    ...
    try:
        # Delete a file attachment
        await api_instance.delete_transaction_attachment(file_id)
    except Exception as e:
...
async with lunchmoney.ApiClient(configuration) as api_client:
    ...
    try:
        # Get a url to download a file attachment
        api_response = await api_instance.get_transaction_attachment_url(file_id)

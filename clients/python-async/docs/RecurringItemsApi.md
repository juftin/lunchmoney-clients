...
async with lunchmoney.ApiClient(configuration) as api_client:
    ...
    try:
        # Get all recurring items
        api_response = await api_instance.get_all_recurring(start_date=start_date, end_date=end_date, include_suggested=include_suggested)
...
async with lunchmoney.ApiClient(configuration) as api_client:
    ...
    try:
        # Get a single recurring item
        api_response = await api_instance.get_recurring_by_id(id, start_date=start_date, end_date=end_date)
